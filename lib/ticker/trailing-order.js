...
class TrailingOrder extends EventEmitter {
   ...
   */
  constructor(security = 'BTC-USD', provider = 'gdax') {
    super();
    this.uuid = uuid();
    this.funds = {};
    this.funds.base = 0.0;
  ...
  validate(order) {
    ...
    debug(`*** ${this.provider}.${this.security}: `);
  ...
  checkTradeSignal(order, index) {
    ...
    order.target = order.target || price;
      ...
      if (this.executeTrade && size >= MIN_TRADE_SIZE) {
        this.executeTrade(tradeParams.trade, (error, response, data) => {
          if (error || (response && response.statusCode !== 200)) {
            ...
            this.emit('error', `Error executing trade. statusCode=${response.statusCode} error=${JSON.stringify(error || data)}. Trade ${JSON.stringify(tradeParams)}`);
          } else {
