              ...
              if (value.event === event.ORDER) {
                ...
                Object.entries(value.order).forEach(([key, order]) => {
                  ...
                  to.on('trade', tradeParams => {
                    observer.next(Object.assign({}, tradeParams, { event: event.TRADE, provider: provider }));
                  });
                ...
                // set price
                const key = `${value.provider}.${value.product_id}`;
                if (orders[key]) {
                  orders[key].setPrice(value.price);
                }
              }
            } catch (err) {
              console.error(new Error(`Error on trailing orders observer: ${err}`));
              // observer.error(err);
            }
          },
