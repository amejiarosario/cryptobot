...
function ticker(providers, options = {}) {
  debug('ticker', providers, options);
      ...
      .subscribe(
        data => {
          mongo.connect((error, db)=> {
            ...
            const collection = db.collection(`${productId.toLowerCase()}-ticker`); // for backward compatibility
            ...
            debug(`Saving ${data.length} docs for ${name}: ${productId}. Buffer time is ${bufferTime/1000} seconds.`);
