...
function ticker(providers) {
  ...
  providers.forEach(({ name, productIds }) => {
    ...
    productIds.forEach(productId => {
      ...
      const trades = Observable.fromEvent(trailingOrder, 'trade')
        .mergeMap(i => Observable.of(Object.assign(i, { event: 'trade', provider: name, productId: productId })));
  ...
  observable = observable.merge(ordersObservable);
