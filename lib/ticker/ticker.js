...
class Ticker {
  ...
  start(onData, onError, onComplete) {
    ...
    this.subscription = this.observables
      ...
      .let(this.logger(data => data.event !== EVENT.TICK))
              ...
              if (value.event === EVENT.ORDER) {
                ...
                Object.entries(value.order).forEach(([key, order]) => {
                  ...
                  if(orders[key]) {
                    ...
                    to = new TrailingOrder(productId, provider);
                ...
                if (agg) {
                  ...
                  const t = setTimeout(() => {
                    if(aggregator[key].data.length > 1) {
                      ...
                      mongo.saveTickAggregation(key, aggregator[key].data, saveSequence++);
                      // .catch(error => {
