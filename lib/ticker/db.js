...
async function saveTickAggregation(productId, ticks) {
  // debug('saveTickAggregation');
  ...
  const promises = Object.values(TIME).map((time, index) => {
    ...
    const p = collection.bulkWrite(bulkUpdateTicks, { ordered: true, w: 1 });

    return p;
  });

  const results = await Promise.all(promises).then(saved => {
    db.close();
    debug(`${productId} bulk operation: %o [verbose]`, JSON.stringify(saved));
  });
 ...
 */
async function getOhlc({tickerId = 'gdax.btc-usd', resolution = TIME.MINUTES, period = 50, multiplier = 1} = {}) {
  debug('getOhlc', tickerId, resolution, multiplier, period);
  ...
  const limit = { $limit: parseInt(period) * parseInt(multiplier) };
  const toArray = {$group: {
    _id: null,
    data: { $push: '$$ROOT' }
  }};
