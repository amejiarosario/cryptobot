...
async function saveTickAggregation(productId, allTicks, saveSequence) {
  debug('saveTickAggregation [verbose]', productId, allTicks.length);
  ...
  for (let index = 0; index < chunks.length; index++) {
    ...
    const results = await Promise.all(promises).then(saved => {
      debug(`${productId} bulk operation #${saveSequence} ${totalChunks > 1 ? '(' + index + '/' + totalChunks + ')' : ''}: %o [verbose]`, JSON.stringify(saved));
    });
  }
