 ...
 * Get OHLC data
 *
 * FIXME: Exclude the current day, because its close value will change with every new value added to it and can't be counted as final
 *
...
async function getOhlc({tickerId = 'gdax.btc-usd', resolution = TIME.MINUTES, limit = 50, multiplier = 1} = {}) {
  ...
  const pipeline = [descOrder, limitResults, toArray, addOrdinal, addGroupId, aggregateData, removeGroupId, addId, removeFirstId, ascOrder];
