...
async function getAbsoluteOhlc({ tickerId = 'gdax.btc-usd', resolution = TIME.MINUTES, limit = 50, multiplier = 1 } = {}) {
  ...
  const output = collection.mapReduce(mapDaysSinceEpoch, reduceOhlc, {
    out: { inline: 1 },
    sort: { timestamp: -1 },
  ...
  const results = _.get(output, 'results', []);

  // transforms { _id, value: ticks } to array of ticks
