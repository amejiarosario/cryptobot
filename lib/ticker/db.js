...
async function saveTickAggregation(productId, ticks) {
  ...
  const promises = Object.values(TIME).map((time, index) => {
    ...
    // console.log('ttt', collectionName, CONFIG.db.uri, JSON.stringify(bulkUpdateTicks))
    const p = collection.bulkWrite(bulkUpdateTicks);
  ...
  debug(`${productId} bulk operation: `, JSON.stringify(results), '[verbose]');
