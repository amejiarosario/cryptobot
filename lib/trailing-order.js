...
class TrailingOrder extends EventEmitter {
  ...
  validate(order) {
    if(order.buy) {
      const { price: buyPrice, trailing: buyTrailing, action: buyAction, percentage: buyPercentage } = order.buy;
      console.log(order.buy);
      console.log(`When PRICE reach ${buyPrice} or lower (+/- ${buyTrailing * 100}%), then BUY ${btc(this.funds.quote * buyPercentage / buyPrice)} BTC with ${usd(this.funds.quote * buyPercentage)} USD (${buyPercentage*100}% of ${this.funds.quote})`);
    }

    if(order.sell) {
      const { price: sellPrice, trailing: sellTrailing, action: sellAction, percentage: sellPercentage } = order.sell;
      console.log(order.sell);
      console.log(`When PRICE reach ${sellPrice} or higher (+/- ${sellTrailing * 100}%), then SELL ${btc(this.funds.base * sellPercentage)} BTC for ${usd(this.funds.base * sellPercentage * sellPrice)} USD (${sellPercentage*100}% of ${this.funds.base})`);
    }
  }
