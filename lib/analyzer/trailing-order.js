...
class TrailingOrder extends EventEmitter {
  ...
  updateTrigger(side, order, trigger) {
    if (!order.trigger[side] ||
      (side === 'buy' && order.trigger[side].trail > trigger.trail ||
      side === 'sell' && order.trigger[side].trail < trigger.trail)
    ) {
      order.trigger[side] = trigger;
      debug(`Trigger for ${side} has change %o --- %o --- to: %o`, this.price, order.trigger, this.toObject());
    }
  }
  ...
  makeTrade({price, side, order, index}) {
    ...
    if (size > 0) {
      ...
      debug('>>> Trading <<<', tradeParams, this.toObject());
