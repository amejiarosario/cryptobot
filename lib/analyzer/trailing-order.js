...
class TrailingOrder extends EventEmitter {
      ...
      if(this.getFunds) {
        this.getFunds((error, response, funds) => {
          if (error || (response && response.statusCode !== 200)) {
            this.emit('error', `Error getting funds. statusCode=${response.statusCode} error=${error || data}`);
          } else {
            this.setFunds(funds);
  ...
  updateTrigger(side, order, trigger) {
    if (!order.trigger[side] ||
      ...
      debug(`Trigger for ${this.security} ${side} @ ${order.target} has change %o --- %o --- to: %o`, this.price, order.trigger, this.uuid);
    }
  }
    ...
    if (size > 0) {
      ...
      if (this.executeTrade) {
        this.executeTrade(tradeParams.trade, (error, response, data) => {
          if (error || (response && response.statusCode !== 200)) {
            this.emit('error', `Error executing trade. statusCode=${response.statusCode} error=${error || data}`);
          } else {
