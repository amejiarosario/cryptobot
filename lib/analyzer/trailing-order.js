...
class TrailingOrder extends EventEmitter {
  ...
  setFunds(opts = {}) {
    ...
    this.funds.quote = usd(opts[this.quoteSymbol] || opts.quote || opts || 0.0);
  }
  ...
  checkTradeSignal(order, index) {
    ...
    if (side === 'buy' && this.trigger.buy && price >= this.trigger.buy.price ||
      side === 'sell' && this.trigger.sell && price <= this.trigger.sell.price) {

      if(this.getFunds) {
        this.getFunds((err, res, funds) => {
          this.setFunds(funds);
          this.makeTrade({ price, side, order, index });
        });
      } else {
        this.makeTrade({ price, side, order, index });
      }
    ...
    if (side === 'sell' && price >= target) {
      ...
      this.updateTrigger(side, trigger);
   ...
   * @param {*} trigger
   */
  updateTrigger(side, trigger) {
    if (!this.trigger[side] ||
      (side === 'buy' && this.trigger[side].trail > trigger.trail ||
      side === 'sell' && this.trigger[side].trail < trigger.trail)
    ) {
      this.trigger[side] = trigger;
      debug(`Trigger for ${side} has change`, this.trigger);
    }
  }

  makeTrade({price, side, order, index}) {
    ...
    if (size > 0) {
      ...
      debug('>>> Trading <<<', tradeParams);

      if (this.executeTrade) {
        this.executeTrade(tradeParams.trade, callback);
      }
