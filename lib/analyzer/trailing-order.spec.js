  ...
  describe('# funds and execute trades', () =>{
    it('should get funds before doing trades', done =>{
      ...
      to.on('trade', params => {
        ...
        sinon.assert.calledWith(trade, { price: 3600, product_id: "BTC-USD", side: "buy", size: 0.20833333});
        expect(to.funds).to.eql({ "base": 0.47384029000000005, "quote": 1462.000012 });
        done();
      });
      ...
      to.setOrder([
        ...
        { "side": "buy", "target": 3600, "trailing": { "amount": 50 }, "trade": { "percentage": 0.5, "amount": 1000 } }
      ]);
