...
function security(tickerId, multiplierResolution, limit, interval) {
  ...
  return Observable.create(observer => {
    ...
    getLastTicks(params, last, observer);
...
function getLastTicks({ tickerId, resolution, multiplier, limit }, last, observer) {
  return mongo.getOhlc({tickerId, resolution, multiplier, limit}).then(dataArray => {
    ...
    if(newData.length > 0) {
      ...
      last._id = newData[newData.length -1]._id;
    }
    return newData;
  }).catch(error => {
...
function getOnlyNewData(data, last) {
  ...
  // debug('%o', { data, last, lastIndex, slice: { s: (lastIndex + 1), e: (data.length), data: data.slice(lastIndex + 1, data.length)}});
  if(lastIndex > -1) {
