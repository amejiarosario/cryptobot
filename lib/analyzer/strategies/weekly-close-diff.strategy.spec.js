...
describe.only('WeeklyCloseDiffStrategy', function () {
  this.timeout(100);
  let strat;
  let observable;
  ...
  describe('# Start', () => {
    ...
    it('should save previous ticks history', () => {
      ...
      observable.next([8].map(t => ({ close: t })));
      expect(exitStub.called).to.eql(true);
      expect(entryStub.called).to.eql(false);
    });
  });
