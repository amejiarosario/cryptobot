...
class WeeklyCloseDiffStrategy {
  ...
  constructor({ tickerId = 'gdax.BTC-USD',
                ...
                trailing = { percentage: 0.05, amount: 15 }, // 5% price or $150 price, whatever is less. e.g. 7,000 BTC/USD => +/- Math.min(350,150); 300 ETH/USD => Math.min(15, 150); 50 LTC/USD => Math.min(2.5, 150)
                trade = { quote: 1000} } = {}) {
    ...
    this.hasOnlyNewData = false; // send available data all the time (without filtering by _id). This is because the moving close price will be different all the time.
  ...
  start() {
    this.subscription = security(this).subscribe(
      ticks => {
        this.closeDiff = new DiffPercentage(this.limit);
