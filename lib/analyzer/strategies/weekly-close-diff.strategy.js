...
class WeeklyCloseDiffStrategy {
  ...
  constructor({ tickerId = 'gdax.BTC-USD',
                ...
                takeProfit = 0.30,
                thresdhold = 0,
                interval = 2 * MINUTES, // recommended between 2 min and 12 hours
                trailing = {},
                trade = {quote: 500}} = {}) {
    ...
    this.closeDiff = new DiffPercentage(period);
  ...
  start() {
    this.subscription = security(this.tickerId, this.resolution, this.period, this.interval).subscribe(
      ticks => {
