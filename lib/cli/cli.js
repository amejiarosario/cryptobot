...
function setupProgram() {
  ...
  program
    ...
    .action((options) => {
      const t = CONFIG.ticker; // default or env settings
      const {provider = t.providers, modifier = t.modifiers} = options || {};

      const ticker = new Ticker(provider, modifier);
      ticker.subscribe();

      process.once('SIGINT', ticker.unsubscribe);
      process.once('SIGTERM', ticker.unsubscribe);
    });
