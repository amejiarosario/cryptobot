 ...
 * $ ps aux | grep -E "ticker|analyzer|ticker,web,analyzer" | grep -v grep | awk 'NR==1 {print $2}' | xargs kill && npm run simulator
 */
...
const END = '2017-09-25T00:00:00.000Z';
...
describe('Weekly close diff strategy simulator', function () {
  ...
  beforeEach(done => {
    ...
    wss = new GdaxWebsocketMock({collection: {
      ...
      delay: 200 // delay sending ws messages
    }});
